...
public interface MyOrderRepository extends PagingAndSortingRepository<MyOrder, Long>, JpaSpecificationExecutor {
    ...
    List<MyOrder> findAllByCustomer_IdAndStatusNotOrderByCreateTimeDesc(Long id,Long status);

    List<MyOrder> findAllByCustomer_Id(Long id);

    List<MyOrder> findAllByRider_Id(Long id);

    List<MyOrder> findAllByStatus(Long status);
}
