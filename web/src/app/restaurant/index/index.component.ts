...
export class IndexComponent implements OnInit {
   ...
   * @param object 用户
   */
  onDelete(object: Restaurant): void {
    ...
    this.commonService.confirm((confirm = false) => {
      if (confirm) {
        ...
        this.fileService.delete(object.user.pictureName)
          .subscribe(() => {
            this.restaurantService.delete(object.id!)
              .subscribe(() => {
   ...
   * @param data 分页数据
   */
  validateData(data: Page<Restaurant>): void {
   ...
   * @param user 用户
   */
  validateUser(user: Restaurant): void {
    // 必有条件
  ...
  authentication(restaurant: Restaurant) {
    restaurant.status = 2;
    this.restaurantService.update(restaurant.id, restaurant)
      .subscribe(() => {
  ...
  statusChange(restaurant: Restaurant) {
    restaurant.status = 3-restaurant.status;
    this.restaurantService.update(restaurant.id, restaurant)
      .subscribe(() => {
        this.commonService.success(() => {
        });
